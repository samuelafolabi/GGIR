...
<div style="background:#f0f4ff;border-left:4px solid #0054AD;padding:1rem 1.2rem;margin-bottom:1.5rem;border-radius:4px">

**⏱ Reading time:** ~20 min &nbsp;|&nbsp; **Chapter:** 9 of 14

**What you will learn**

- How GGIR defines the candidate sleep window (the “guider”)
- The available guider algorithms and when to use each
- How to configure guiders with GGIR parameters, including sleep log integration

**Prerequisites**

- Read the previous chapter: [Chapter 8: Sleep Fundamentals: SIB Detection](chapter8_SleepFundamentalsSibs.html)
- 📄 [van Hees et al. 2018: HDCZA algorithm](https://dx.doi.org/10.1038/s41598-018-31266-z)


**Key terms:** <abbr title="A method used by GGIR to estimate the candidate main sleep window each night">guider</abbr>, <abbr title="A guider that identifies the longest period of limited postural change per day as the candidate sleep window; default for wrist-worn data">HDCZA</abbr>, <abbr title="The time window from sleep onset to the end of the last SIB within the guider window">SPT (Sleep Period Time window)</abbr>, <abbr title="The period from going to bed to getting out of bed; used when a sleep log records time in bed rather than SPT">TimeInBed</abbr>, <abbr title="A self-reported diary recording sleep onset and wake-up times; the preferred guider when available">sleep log</abbr>, <abbr title="The GGIR parameter selecting the guider algorithm">HASPT.algo</abbr>, <abbr title="A participant whose main sleep period starts before and ends after noon (e.g., shift worker)">daysleeper</abbr>, <abbr title="An experimental guider for hip-worn devices that detects the longest period with horizontal trunk orientation">HorAngle</abbr>, <abbr title="An experimental guider that detects the longest rest bout; for count data">HLRB</abbr>, <abbr title="A continuous period of limited body movement used as a proxy for sleep">SIB (Sustained Inactivity Bout)</abbr>
...
3.  Assess overlap between the windows identified in step 1 and 2, which we use to define the Sleep Period Time window (<abbr title="The time window from sleep onset to the end of the last SIB within the guider window">SPT</abbr>) or the time in bed window (<abbr title="The period from going to bed to getting out of bed; used when a sleep log records time in bed rather than SPT">TimeInBed</abbr>) as discussed in chapter 10.
...
We will refer to the methods for defining the window as **<abbr title="A method used by GGIR to estimate the candidate main sleep window each night">guiders</abbr>**.
The following guiders are available:

If a <abbr title="A self-reported diary recording sleep onset and wake-up times; the preferred guider when available">sleep log</abbr> is provided, GGIR uses by default guider sleep log, if the sleep log is not available it falls back on the algorithm as specified by parameter <abbr title="The GGIR parameter selecting the guider algorithm">`HASPT.algo`</abbr>, which allows tps ecify any of the algorithms are discussed below (default HDCZA).
...
In this way our method is sensitive to individuals who have their main sleep period starting before noon and ending after noon, referred as **<abbr title="A participant whose main sleep period starts before and ends after noon (e.g., shift worker)">daysleepers</abbr>** in the output.
...
The <abbr title="A guider that identifies the longest period of limited postural change per day as the candidate sleep window; default for wrist-worn data">HDCZA</abbr> algorithm is designed for studies with wrist-worn accelerometer (raw) data where no sleep log is available. The algorithm was first described in a [2018 article](https://dx.doi.org/10.1038/s41598-018-31266-z), and has been modified slightly: Step 6 in Figure 1 has been replaced by a single threshold (0.2 by default).\
...
<abbr title="An experimental guider that detects the longest rest bout; for count data">HLRB</abbr> is a heuristic algorithm to detect the Longest Rest Bout. It was designed to offer a guider option for accelerometer device that only store count data and by that cannot work with the HDCZA or HorAngle guiders.
...
If this is used it will also define the resulting window as <abbr title="A continuous period of limited body movement used as a proxy for sleep">SIB</abbr> period and ignore all other identified SIB window to ensure the entire window is treated as sleep. So, all SIB periods detected are ignored.
...
## Automated correction of acceleration-based guiders

In our experience guiders that rely on accelerometer data such as HDCZA can struggle with the following scenarios:

- Unexpected large gaps in the main sleep window resulting, which are not tolerated and resulting in the detection of only one half of the sleep period time window.
- Sleep is fragmented and the person has a long nap in the middle of the day resulting in afternoon nap being labelled as the main sleep window.

To address this an algorithm has been proposed. The algorithm is currently being optimised and this part of the documentation will be updated once the optimisation has been finalised. The algorithm has mainly been tested with the HDCZA guider and current default parameter values were optimised for one specific clinical study population. Therefore, the default correction algorithm configuration should be used with caution and we encourage you to help evaluate its appropriateness for various study populations and guider algorithms. Please note that the correction algorithm is only used for guiders that rely on accelerometer data, as present in GGIR part 3 such as HDCZA, and not for the sleeplogs.

To turn on the correction algorithm set `guider_cor_do=TRUE`.

### Correction algorithm 

The algorithm involves the following steps:

1. Apply default guider (e.g. HDCZA) to each night, but retain both the longest block and refer to it as the main guider window as well as all other candidate sleep blocks that are normally omitted.
2. Identify min-max window running from the minimum sleep onset to maximum wakeup time from all main guider windows (longest block) across all nights in one recording.
3. If there are at least 3 (parameter: `guider_cor_meme_min_dys`) valid nights:
    a. Identify median-median window running from the median sleep onset until the median wakeup time from all main guider windows across all nights in one recording.
    b. Loop over valid nights and select an alternative main guider window if the default main guider window is >90% outside the median-median window (parameter: `guider_cor_meme_frac_out`) and there is a secondary guider-identified sleep block that:
        i. overlaps >40% with the median-median window (parameter: `guider_cor_meme_frac_in`)
        ii. lasts at least 2 hours (parameter: `guider_cor_meme_min_hrs`)
        iii. has at least 80% sustained inactivity bouts (parameter: `guider_cor_min_frac_sib`).
    c. Identify min-max window (step 1) again but now based on the corrected main guider window from step 2b.
4. Loop over nights and expand the main guider window towards neighbouring guider-identified sleep blocks if those secondary sleep blocks:
    a. Overlap  with the min-max window (any duration).
    b. Lasts > 1 hour (parameter: `guider_cor_min_hrs`).
    c. Have > 80% sustained inactivity bouts (parameter: `guider_cor_min_frac_sib`).
    d. Are not separated from the main guider window by more than 3 hours (parameter: `guider_cor_maxgap_hrs`).

Step 3 comes with the assumption that the main sleep period typically takes place in the same part of the day within the same individual. If you disagree and would like to skip step 3 set parameter `guider_cor_meme_min_dys = Inf`.

### Tracing whether correction algorithm was applied

In the part4_nightsummary output (csv file) you will find a column named guider_corrected. This variable can have the following values:

- Value 0: Step 1 and 2 ran, but none of the conditions for step 3 and 4 were met and no corrections were made.
- Value 1: Step 4 correction was made.
- Value 2: Step 3 correction was made.
- Value 3: Step 3 and 4 corrections were both made.

### Guidance on how to change parameter values:

| Parameter | Default	| Reduce risk of missing half of the night? | Reduce risk of detecting daytime nap as main sleep window? |
|------------------------|--------|-------------------------|---------------------------|
| guider_cor_maxgap_hrs	   | 3	| Increase value | NA |
| guider_cor_min_frac_sib  |	0.8	| Decrease value | NA |
| guider_cor_min_hrs	     | 1 | Decrease value | NA |
| guider_cor_meme_frac_out | 0.9 | NA	| Decrease value |
| guider_cor_meme_frac_in  | 0.4 |	NA | Decrease value |
| guider_cor_meme_min_hrs  | 2 |	NA | Decrease value |
...
GGIR(
  ...
  HASPT.algo = "HDCZA",        # use HDCZA guider for wrist-worn data (default)
  HASIB.algo = "vanHees2015"   # SIB algorithm (default)
)
```

---

<div style="background:#f8f9fa;border-top:2px solid #dee2e6;padding:1rem 1.2rem;margin-top:2rem;border-radius:4px">

### Summary

GGIR uses a “guider” to identify the candidate main sleep window each night before overlapping it with SIBs to define the SPT. The default guider for wrist-worn data is HDCZA. When a sleep log is available, it takes priority. A daysleeper shift (noon–noon → 6pm–6pm) is applied automatically when needed.

### What to read next

➡ [Chapter 10: Sleep Analysis](chapter10_SleepAnalysis.html)
...
- 📹 [Full GGIR tutorial playlist](https://www.youtube.com/watch?v=AbHgJYTyuiA)
