...
**⏱ Reading time:** ~25 min &nbsp;|&nbsp; **Chapter:** 2 of 14

**What you will learn**

- How the GGIR pipeline is structured into six parts and what each part does
- How to invoke GGIR with a single function call and configure it with parameters
- How milestone data enables efficient, resumable processing across multiple files
- How to work with externally derived epoch-level data

**Prerequisites**

- Read the previous chapter: [Chapter 1: What is GGIR?](chapter1_WhatIsGGIR.html)


**Key terms:** <abbr title="Intermediate RData files saved between GGIR pipeline parts to allow resuming without re-running earlier steps">milestone data</abbr>, <abbr title="A fixed-length time window (default 5 s) over which raw acceleration is summarised into a metric value">epoch</abbr>, <abbr title="Acceleration data stored in gravitational units at the native sampling rate of the sensor">raw accelerometer data</abbr>, <abbr title="The six sequential computational stages of the GGIR pipeline (parts 1–6)">Parts (GGIR pipeline)</abbr>, <abbr title="Time periods when the accelerometer was not worn, detected via standard deviation and range of the raw signal over 60-minute windows">non-wear</abbr>
...
Each part, when run, stores its output in an R-data file that has `.RData` as its extension, which we refer to as the <abbr title="Intermediate RData files saved by each GGIR part allowing pipeline resumption">milestone data</abbr>. As user, you are unlikely to ever need to interact directly with the milestone data files because all relevant output for you is stored in csv, png and pdf files in the output folder `results`.
...
-   **params_metrics:** parameters related to aggregating the raw data to <abbr title="A fixed-length time window (default 5 s) over which raw acceleration is summarised">epoch</abbr> level summary measures (metrics).
...
GGIR automatically stores all its parameter settings in a csv-file named config.csv. The file is stored after each run of GGIR in the root of the output folder, overwriting any existing config.csv file. So, it acts as log of how GGIR was configured during the most recent run.

One practical value of the configuration file is that you can share it with others such that they can replicate your analyses via GGIR parameter `configfile`. The code below shows an example where `"D:/myconfigfiles/config.csv"` is used as configuration file. Note that parameters `datadir` and `outputdir` discussed below are stored in the configuration file but always need to be specified directly in R code as they are never extracted from the configuration file.
...
Be aware that, to replicate GGIR analysis, one should also make sure that consistent GGIR and R versions are used. See [here](https://wadpac.github.io/GGIR/articles/chapter0_Installation.html) for guidance on how to install older package versions.
...
-   [Parmay Matrix](https://www.matrixwearable.net/) .bin and .BIN
...
To process these files, GGIR loads their content and saves it as GGIR part 1 milestone data, essentially fooling the rest of GGIR to think that GGIR part 1 created them based on raw data input. As will be discussed in chapter 3, GGIR does <abbr title="Time periods during which the accelerometer is not worn">non-wear</abbr> detection in two steps: The first step is done in part 1 and the second step is done in part 2. In relation to externally derived epoch data non-wear is detected by looking for consecutive zeros of one hour (default) expanded by all time point for which data is missing. However, the consecutive zero approach is not used for all brands:
...
library(GGIR)
GGIR(
  datadir   = "C:/mystudy/mydata",   # folder with accelerometer files
  outputdir = "D:/myresults",         # where GGIR saves all output
  do.report = c(2, 4, 5)             # generate summary reports for parts 2, 4, 5
)
```

This single call runs all six pipeline parts in sequence and saves milestone data so that individual parts can be re-run without restarting from scratch.
...
The GGIR pipeline consists of six sequential parts. A single `GGIR()` call with at minimum `datadir` and `outputdir` runs all parts and saves intermediate milestone data for each recording. Parameters are passed directly to the `GGIR()` function or via a `config.csv` file.

### What to read next

➡ [Chapter 3: Data Quality Assurance](chapter3_QualityAssessment.html)
...
<div style="position: relative; padding-bottom: 56.25%; height: 0; overflow: hidden; max-width: 100%; border-radius: 4px; margin-bottom: 1.5rem;">
  <iframe data-external="1" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%;" src="https://www.youtube.com/embed/kbCU5uEif34?si=obvs5J48ud7eKao6" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
</div>
